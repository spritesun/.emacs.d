...
;; Author: ctlaltdel <ctlaltdel@geocities.co.jp>, lambda_list <lambda_list@hotmail.com>
...
;;[Commentary]
;;
;;	This is a MSN Messenger Client (for people who need no Japanese support).
;;	diff from original (Japanese) version is
;;	 - comment translated to English from Japanese.
;;	 - Remove Japanese from this file.
;;	 - older (but stable ... probably)
;;
;;	Latest version available from
;;
;;	http://www.geocities.co.jp/SiliconValley-SanJose/7474/      (Japanese)
;;	http://members.tripod.co.jp/lambda_list/EmacsMessenger.html (English)

;;[Requirement]
;;
;;	* Hotmail Account :-)
;;	* Emacs 21 or later
;;	* Mule-UCS 0.84 (if you don't use Japanese, it's needless)
;;
...
;;
;;	*experimental*
;;	if you use file transfer function, you need `msnftp' command.
;;	it's available from
;;	 http://www.geocities.co.jp/SiliconValley-SanJose/7474/download/msnftp.c.txt
;;	(tested in FreeBSD 4.7 RELEASE & WindowsXP Professional + mingw)
...
;;[How To Use]
;;	To install, put this file in your load-path and add the following to your .emacs
;; 	(load "messenger-e.el")
;;	M-x msn-mode	start messenger mode.
;;			Please wait untill login message.
;;	 C-cC-o NLN	change status to ONLINE
;;	 C-cC-n		create session
;;	 C-cC-t		select session
;;	 mouse-1	select session from 'ssession: [0]' line
;;	 C-cC-r		call someone to current session
;;	 C-cC-c		send message
;;	 C-cC-q		exit messenger mode
;;	 msn-rename	change friendly name
;;	 C-cC-s		sync contact list
;;	*experimental*
;;	 msn-receive-file	 receive file
...
(defvar *msn-version* "MSN Messenger on Emacs version 0.0.1.1e")
...
(defvar *msn-experimental* t "experimental feature")
  ...
  (if (null *msn-current-session*)
	...
	(delete-process session))
      ...
      (msn-show-command *msn-current-session*
			(concat " close <" *msn-current-session* ">"))
...
(defun msn-ring ()
  ...
  (if (null (and *msn-current-session*
      ...
      (message "current session unavailable X-(")
  ...
  (if (null *msn-current-session*)
    ...
    (let ((session (cdr (assoc (string-to-int *msn-current-session*) *msn-switchboards*))))
      (when (and session
		 ...
		 (eq (process-status session) 'open))
	(let* ((filename (expand-file-name (read-file-name "File: " (expand-file-name "~/") nil t)))
...
(defun msn-initialize-screen ()
  ...
  (let ((inhibit-read-only t))
    ...
    ;; Contact-list part.
    (msn-insert-with-face 'default "\n")
    (msn-insert-with-face 'msn-status-delimiter-face  " [Contact Lis]\n")))
...
(defun msn-invited (session name mail body)
  (let ((lst (split-string body "\n")) 
	...
	(msnftp-recv ip port auth)))
	 ...
	 (msn-insert-with-face
	  'msn-session-header-face
	  (concat "<" session "> " "cancel from "  name " <" mail "> " "\n")))))
	 ...
	 (msn-insert-with-face
	  'msn-session-header-face
	  (concat "<" session "> file from " name " <" mail "> " "\n"))
	 (msn-insert-with-face 
...
(defun msnftp-sentinel (process event)
  (when event
	...
	(msn-show-command nil "[SUCEESS] file transfer"))
       ((= status 1)
	(setq *msn-sending-file* nil)
	(msn-show-command nil "[FAILED] file transfer"))))))
	  ...
	  (cond
	   ((string-match "^ILN" string)
	    (let* ((lst (split-string string "[ \n]"))
	      ...
	      (msn-show-command nil (concat name " <" mail "> : (" (msn-status-expand stat) ")"))))
	   ...
	   ((string-match "^OUT" string) (msn-show-command nil "connection reset from server"))
	   ;; to Contact-list
	   ((string-match "^GTC" string)
	    ...
	    (cond
	     ...
	     ((string-match "^ILN" string)
	      (let* ((lst (split-string string "[ \n]"))
		     ...
		     (mail (nth 3 lst))
		     (name (nth 4 lst)))
		(msn-show-command session (concat "<" session "> [JOIN]" name " <" mail ">"))
		(msn-set-contact-list mail stat name)))
...
;; ;; CMUCL 1.8d on FreeBSD 4.7 RELEASE
;;(do* ((i 0 (+ i 1.0d0))
